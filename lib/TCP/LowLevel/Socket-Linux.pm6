...
use StrictClass;
unit class TCP::LowLevel::Socket-Linux:ver<0.0.1>:auth<cpan:JMASLAK>
    does StrictClass;
...
method connect(Str:D $host, Int:D $port where ^(2¹⁶), Bool :$bin -->Promise:D) {
    ...
    start {
        ...
        if $rv {
            ...
            $!state = SOCKET_BOUND;
            my $conn = TCP::LowLevel::Socket-Connection-Linux.new(
                :my-host($!my-host),
...
# XXX Check out leaving information, quit when appropriate.
method acceptor(Bool :$bin -->Supply:D) {
